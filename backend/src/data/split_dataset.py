...
def split_train_test(dataset: pd.DataFrame, **kwargs
    ...
    df_train.to_csv(
        kwargs["train_path_proc"], sep=kwargs["data_separator"], index=False
    )
    df_test.to_csv(kwargs["test_path_proc"], sep=kwargs["data_separator"], index=False)
...
def get_train_test_data(
    ...
    """
    Получение train/test данных разбитых по отдельности
    на объект-признаки и целевую переменную
